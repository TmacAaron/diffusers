...
class FluxAttnProcessor:
    ...
    def __call__(
        ...
        if hasattr(self._parallel_config, "context_parallel_config") and \
            ...
            return self._context_parallel_forward_vqk(
    ...
    def _context_parallel_forward_cv(
        self,
        attn: "FluxAttention",
        hidden_states: torch.Tensor,
        encoder_hidden_states: torch.Tensor = None,
        ...
        value = value.reshape(B, S_KV_LOCAL, world_size, H_LOCAL, D).permute(2, 1, 0, 3, 4).contiguous().flatten()
        v_size = value.numel()
        ev_v.record()
        ...
        with torch.npu.stream(STREAM_VECTOR):
            ...
            query = query.reshape(B, S_Q_LOCAL, world_size, H_LOCAL, D).permute(2, 1, 0, 3, 4).contiguous().flatten()
            key = key.reshape(B, S_KV_LOCAL, world_size, H_LOCAL, D).permute(2, 1, 0, 3, 4).contiguous().flatten()
            q_size = query.numel()
            k_size = query.numel()

            ev_v.wait()
            qkv_all = funcol.all_to_all_single(torch.cat([query, key, value], dim=0), None, None, group)
        
        qkv_all = _wait_tensor(qkv_all)
        query_all = qkv_all[:q_size].reshape(world_size, S_Q_LOCAL, B, H_LOCAL, D).flatten(0, 1).permute(1, 0, 2, 3).transpose(2,1).contiguous()
    ...
    def _context_parallel_forward_vqk(
        self,
        attn: "FluxAttention",
        hidden_states: torch.Tensor,
        encoder_hidden_states: torch.Tensor = None,
